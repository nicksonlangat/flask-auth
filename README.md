...
The following `curl` command registers a new user with username `nick` and password `python`:

    $ curl -i -X POST -H "Content-Type: application/json" -d '{"username":"nick","password":"python"}' http://127.0.0.1:5000/api/users
...
These credentials can now be used to access protected resources:
    ...
    $ curl -u nick:python -i -X GET http://127.0.0.1:5000/api/resource
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 30
    Server: Werkzeug/0.9.4 Python/2.7.3
...
Using the wrong credentials the request is refused:
    ...
    $ curl -u nick:ruby -i -X GET http://127.0.0.1:5000/api/resource
...
Finally, to avoid sending username and password with every request an authentication token can be requested:
    ...
    $ curl -u nick:python -i -X GET http://127.0.0.1:5000/api/token
    HTTP/1.0 200 OK
    Content-Type: application/json
    Content-Length: 139
    Server: Werkzeug/0.9.4 Python/2.7.3
